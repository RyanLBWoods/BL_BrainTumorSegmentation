...
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, AveragePooling2D, ZeroPadding2D, Dropout, Flatten, Reshape, \
    Activation, Lambda, GlobalAveragePooling2D, GlobalMaxPool2D, add
...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2)):
    ...
    x = Conv2D(nb_filter1, (1, 1), subsample=strides, name=conv_name_base + '2a', use_bias=False)(input_tensor)
    x = BatchNormalization(epsilon=eps, axis=bn_axis, name=bn_name_base + '2a')(x)
    x = Scale(axis=bn_axis, name=scale_name_base + '2a')(x)
    x = Activation('relu', name=conv_name_base + '2a_relu')(x)
    ...
    shortcut = Conv2D(nb_filter3, (1, 1), subsample=strides, name=conv_name_base + '1', use_bias=False)(input_tensor)
...
def resnet101_model(include_top=True, weights='imagenet', input_tensor=None, input_shape=None, pooling=None,
                    classes=1000):

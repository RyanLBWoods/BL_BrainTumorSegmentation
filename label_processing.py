...
def whole_tumor_label(gt_path_list):
    ...
    for path in gt_path_list:
        ...
        for slice in seg_data:
            processed_data = np.zeros(shape=(shape[1], shape[2]))
            for row in range(len(slice)):
                for col in range(len(slice[row])):
                    if slice[row][col] != 0:
                        processed_data[row][col] = 1
            patient_gt.append(processed_data)
        whole_tumor_label_list.append(patient_gt)
    whole_tumor_label_list = np.array(whole_tumor_label_list)
    return whole_tumor_label_list


whole_tumor = whole_tumor_label(gt_path)
print(whole_tumor.shape)

for patient in whole_tumor:
    for d in patient:
